...
def display_simulation_summary(args: argparse.Namespace, cmd_runner: CommandRunner) -> None:
    ...
    if cmd_runner.colored_output:
        print(f"\n{TermColors.CYAN}{stars}{TermColors.ENDC}")
        print(f"{TermColors.CYAN}{TermColors.BOLD}SIMULATION COMPLETE - NO CHANGES WERE MADE{TermColors.ENDC}")
        print(f"{TermColors.CYAN}{stars}{TermColors.ENDC}\n")
        
        print(f"{TermColors.GREEN}The following operations would have been performed:{TermColors.ENDC}")
        print(report)
        
        print(f"\n{TermColors.CYAN}To execute these operations for real, run without the --simulate flag.{TermColors.ENDC}")
    else:
        print(f"\n{stars}")
